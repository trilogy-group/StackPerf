...
- **Ephemeral collector mapping** (default): The collector builds an in-memory map from LiteLLM virtual key ID (`sk-...`) to `proxy_key_id` at startup. Because the `proxy_keys` registry does not store the raw `sk-...` value, the collector must first obtain the `sk-...` -> `key_alias` mapping from an external source (e.g., a live LiteLLM admin API query or an environment variable mapping), then join to `proxy_keys` by `key_alias` to resolve the stable `proxy_key_id`. The resulting map is held only in memory and rebuilt at each startup. The raw `sk-...` value is used only during this resolution, never written.
