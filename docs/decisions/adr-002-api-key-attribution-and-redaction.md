...
- **Ephemeral collector mapping** (default): The collector holds an in-memory map from LiteLLM virtual key ID to `proxy_key_id` that is populated at startup from the `proxy_keys` registry (for example, by a live LiteLLM admin API lookup or a sidecar process). The raw `sk-...` value is used only during resolution, never written.
